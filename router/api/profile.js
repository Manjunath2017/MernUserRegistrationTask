...
router.get('/', async (request, response) => {
  try {
    const profiles = await ProfileModel.find().populate('user', [
      'name',
      'avatar',
    ]) //get only name and avatar field from user collection
...
router.delete('/', auth, async (req, res) => {
  ...
  try {
    //authUserData means user only
    //// Remove profile
    let deleteProfile = await ProfileModel.findOneAndRemove({
      user: res.authUserData.id,
    })
    // console.log('165 line---> \n', deleteProfile)

    //// Remove user
    let deleteUser = await userModel.findOneAndRemove({
      _id: res.authUserData.id,
    })
    // console.log('171 line---> \n', deleteUser)
...
router.delete('/school/:edu_id', auth, async (req, res) => {
  try {
    ////Get a record
    const profile = await ProfileModel.findOne({ user: res.authUserData.id })
    console.log(res.authUserData.id, req.params.edu_id)
    ...
    const removeIndex = profile.education
      .map((item) => item.id)
      .indexOf(req.params.edu_id) //// existing, deleting _id
