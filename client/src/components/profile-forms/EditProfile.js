...
const EditProfile = ({profile:{profile, loading}, auth, createProfileFn, getCurrentProfile, history}) => {
  ...
  const [dispalySocialInputs, toggleSocialInput] = useState(false);
  ...
  return (
    <Fragment>
      ...
      <form className="form" onSubmit={e=>submitFormData(e)}>
        ...
        <div className="my-2">
          <button type="button" onClick={() => toggleSocialInput(!dispalySocialInputs)} className="btn btn-light">
            {!dispalySocialInputs ? 'Show Social Network Links':'Hide Social Network Links'} 
          </button>
          <span>Optional</span>
        </div>
...
EditProfile.propsTypes = {
  createProfileFn:PropTypes.func.isRequired,
...
export default connect(mapStateToProps, {createProfileFn, getCurrentProfile})(withRouter(EditProfile));
